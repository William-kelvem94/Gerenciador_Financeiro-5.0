...
export function LoginPage() {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (!credential) throw new Error('Credencial Google inválida');
      // Obter token do Firebase
      const token = await result.user.getIdToken();
      // Enviar token para backend para autenticação
      ...
      // Atualizar Zustand
      // Se usar setUser do store:
      // useAuthStore.getState().setUser(user, backendToken);
      window.location.href = '/dashboard';
    } catch (error: any) {
      console.error('❌ Erro no login Google:', error);
      toast.error(error?.message || 'Erro no login Google');
    } finally {
      setLoading(false);
    }
  };
