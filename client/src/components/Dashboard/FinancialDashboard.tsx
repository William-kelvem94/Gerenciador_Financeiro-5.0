...
const FinancialDashboard: React.FC = () => {
  ...
  const processedSummary = useMemo(() => {
    ...
    const monthlyGrowth =
      lastBalance !== 0 ? ((currentBalance - lastBalance) / Math.abs(lastBalance)) * 100 : 0;
  ...
  const barChartData = useMemo(() => {
    ...
    const sortedMonths = Object.keys(monthlyData)
      .sort((a, b) => {
        ...
        return (
          new Date(`20${yearA}-${monthA}-01`).getTime() -
          new Date(`20${yearB}-${monthB}-01`).getTime()
        );
      })
  ...
  const lineChartData = useMemo(() => {
    ...
    const dailyBalance = transactions
      ...
      .reduce(
        (acc: { date: string; balance: number }[], tx: any) => {
  ...
  return (
    <div className="dashboard-container bg-gradient-cyber shadow-cyber border-cyber-primary/20 animate-float space-y-8 rounded-xl border p-6">
      {/* Cards de Resumo Cyberpunk */}
      <div className="mb-8 grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-4">
        <CyberpunkCard className="stat-card glass animate-glow">
          <div className="stat-content">
            <h3 className="stat-title text-cyber-primary mb-2 text-lg font-bold">Saldo (Mês)</h3>
            <p
              className={`stat-value font-mono text-3xl ${summary && summary.balance >= 0 ? 'text-cyber-accent' : 'text-cyber-danger'}`}
            >
              R${' '}
            ...
            <span className="stat-change text-cyber-secondary font-mono text-sm">
        ...
        <CyberpunkCard className="stat-card glass animate-glow">
          <div className="stat-content">
            <h3 className="stat-title text-cyber-primary mb-2 text-lg font-bold">Receitas (Mês)</h3>
            <p className="stat-value text-cyber-accent font-mono text-3xl">
              R${' '}
              {new Intl.NumberFormat('pt-BR', { minimumFractionDigits: 2 }).format(
                summary?.totalIncome || 0
              )}
            </p>
            <span className="stat-subtitle text-cyber-primary font-mono text-xs">
        ...
        <CyberpunkCard className="stat-card glass animate-glow">
          <div className="stat-content">
            <h3 className="stat-title text-cyber-primary mb-2 text-lg font-bold">Despesas (Mês)</h3>
            <p className="stat-value text-cyber-danger font-mono text-3xl">
              R${' '}
              {new Intl.NumberFormat('pt-BR', { minimumFractionDigits: 2 }).format(
                summary?.totalExpenses || 0
              )}
            </p>
            <span className="stat-subtitle text-cyber-secondary font-mono text-xs">
        ...
        <CyberpunkCard className="stat-card glass animate-glow">
          <div className="stat-content">
            <h3 className="stat-title text-cyber-primary mb-2 text-lg font-bold">Meta do Mês</h3>
      ...
      <div className="mt-8 grid grid-cols-1 gap-6 md:grid-cols-2">
        <CyberpunkCard className="alert-card glass animate-pulse-neon">
          <h3 className="alert-title text-cyber-accent mb-2 text-lg font-bold">
            🤖 Insights da IA
          </h3>
          <div className="alert-content text-white-muted space-y-1 font-mono">
            <p>
              • Você gastou <span className="text-cyber-accent">15% menos</span> em alimentação este
              mês
            </p>
            <p>
              • Maior gasto: <span className="text-cyber-danger">Compras online (R$ 890,00)</span>
            </p>
            <p>
              • Sugestão:{' '}
              <span className="text-cyber-warning">Considere investir R$ 500,00 extras</span>
            </p>
          </div>
        </CyberpunkCard>

        <CyberpunkCard className="alert-card glass animate-pulse-neon">
          <h3 className="alert-title text-cyber-danger mb-2 text-lg font-bold">⚠️ Alertas</h3>
          <div className="alert-content text-white-muted space-y-1 font-mono">
            <p>
              • Fatura do cartão vence em <span className="text-cyber-warning">3 dias</span>
            </p>
            <p>
              • Meta de gastos em <span className="text-cyber-danger">85%</span> (restam{' '}
              <span className="text-cyber-accent">R$ 375,00</span>)
            </p>
            <p>
              • <span className="text-cyber-secondary">2 transações não categorizadas</span>
            </p>
          </div>
        </CyberpunkCard>
      </div>
