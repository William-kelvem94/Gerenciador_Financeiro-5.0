...
async function main() {
  ...
  try {
    ...
    await prisma.$transaction(async (tx) => {
      ...
      // Create default categories (avoid duplicates)
      ...
      // Get categories for transactions (ensure they exist)
      const foodCategory = await tx.category.findFirst({ where: { name: 'Food & Dining' } });
      const salaryCategory = await tx.category.findFirst({ where: { name: 'Salary' } });
      const transportCategory = await tx.category.findFirst({ where: { name: 'Transportation' } });
      const billsCategory = await tx.category.findFirst({ where: { name: 'Bills & Utilities' } });

      if (!foodCategory || !salaryCategory || !transportCategory || !billsCategory) {
        throw new Error('Required categories not found. Make sure categories are created first.');
      }
